...
function set_temperture!(ps,temp)
    now_temp = mapreduce(p->norm(p.v)^2,+,ps)/length(ps)
    for p in ps
        p.v = kB/ϵ*temp/now_temp*p.v
    end
end   
...
gif(anim,"results/deposition_Ar_80K.gif")
